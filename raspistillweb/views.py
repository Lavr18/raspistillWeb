...
button = False
button_indicator = 0 #for controlling the button enable indicator
...
def settings_view(request):
    ...
    return {'project' : 'raspistillWeb',
            'hostName' : host_name,
            'autostart_enabled' : str(app_settings.autostart_enabled),
            'button_enabled' : str(app_settings.button_enabled),
...
def home_view(request):
    ...
    #Activate the button if needed
    app_settings = DBSession.query(Settings).first()
    if app_settings.button_enabled == 'Yes' and button == False:   
        button_start()
...
def save_view(request):
    global preferences_success_alert, preferences_fail_alert, button, button_indicator 

    autostart_enabled_temp = request.params['autostartEnabled']
    button_enabled_temp = request.params['buttonEnabled']
    ...
    if autostart_enabled_temp:
        app_settings.autostart_enabled = autostart_enabled_temp
        if app_settings.autostart_enabled == 'Yes':
            add_line(autostart_location, line)
        elif app_settings.autostart_enabled == 'No':
            remove_line(autostart_location, line)
            
    
    if button_enabled_temp:
        app_settings.button_enabled = button_enabled_temp
        #If the setting for button is 'yes' and it was not activated before, start button thread
        if app_settings.button_enabled == 'Yes' and button == False:
            button_start()

        elif app_settings.button_enabled == 'No':
            button = False
            button_indicator = 0
...
def button_loop():
    global THUMBNAIL_DIRECTORY, THUMBNAIL_SIZE, timelapse, button_indicator 
    GPIO.setmode(GPIO.BCM)
    GPIO.setup(18, GPIO.IN, pull_up_down=GPIO.PUD_UP) #for Button
    GPIO.setup(23, GPIO.OUT) #for LED
    print ('Button loop start')
    
    while button:
        input_state = GPIO.input(18)
        if input_state == False and timelapse == False:
            button_indicator = 1 #turn on the indicator 'button enabled'
...
def add_line(file_location, line_to_add):
    # Check if the line already exist
    f = open(file_location, "r")
    flines = f.readlines()
    res = filter(lambda x: line_to_add in x, flines)
    f.close
    if len(res) == 0:
        # Add if the line doesn't exist
        f = open(file_location, "a")
        f.write(line_to_add)
        f.close()
        
def remove_line(file_location, line_to_remove):
    f = open(file_location,"r")
    d = f.readlines()
    f.close()
